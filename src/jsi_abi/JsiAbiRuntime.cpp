...
//==============================================================================
// AbiInstrumentation — wraps jsi_instrumentation_vtable
//==============================================================================

class JsiAbiRuntime::AbiInstrumentation
    : public facebook::jsi::Instrumentation {
public:
  AbiInstrumentation(const jsi_instrumentation_vtable *vt, void *inst)
      : vt_(vt), inst_(inst) {}

  ~AbiInstrumentation() override {
    if (vt_ && inst_)
      vt_->release(inst_);
  }
  ...
  std::string getRecordedGCStats() override {
    StringByteBuffer gb;
    vt_->get_recorded_gc_stats(inst_, &gb);
  ...
  std::unordered_map<std::string, int64_t> getHeapInfo(
    ...
    vt_->get_heap_info(
        inst_, includeExpensive,
        [](void *ud, const char *key, int64_t value) {
          auto *map =
              static_cast<std::unordered_map<std::string, int64_t> *>(ud);
          (*map)[key] = value;
        },
        &result);
    return result;
  ...
  void collectGarbage(std::string cause) override {
    vt_->collect_garbage(inst_, cause.c_str());
  }
  ...
  void startTrackingHeapObjectStackTraces(
    ...
    vt_->start_tracking_heap_object_stack_traces(inst_);
  }

  void stopTrackingHeapObjectStackTraces() override {
    vt_->stop_tracking_heap_object_stack_traces(inst_);
  }

  void startHeapSampling(size_t samplingInterval) override {
    vt_->start_heap_sampling(inst_, samplingInterval);
  }

  void stopHeapSampling(std::ostream &os) override {
    vt_->stop_heap_sampling(
        inst_,
        [](void *ud, const char *data, size_t size) {
          auto *stream = static_cast<std::ostream *>(ud);
  ...
  void createSnapshotToFile(
    ...
    vt_->create_snapshot_to_file(inst_, path.c_str(),
  ...
  void createSnapshotToStream(
      std::ostream &os,
      const HeapSnapshotOptions &options = {false}) override {
    vt_->create_snapshot_to_stream(
        inst_, options.captureNumericValue,
        [](void *ud, const char *data, size_t size) {
          auto *stream = static_cast<std::ostream *>(ud);
          stream->write(data, static_cast<std::streamsize>(size));
        },
  ...
  void createSnapshotToFile(const std::string &path) override {
    vt_->create_snapshot_to_file(inst_, path.c_str(), false);
  }
  ...
  std::string flushAndDisableBridgeTrafficTrace() override {
    StringByteBuffer gb;
    vt_->flush_and_disable_bridge_traffic_trace(inst_, &gb);
    return std::move(gb).get();
  }

  void writeBasicBlockProfileTraceToFile(
      const std::string &fileName) const override {
    vt_->write_basic_block_profile_trace_to_file(inst_, fileName.c_str());
  }

  void dumpProfilerSymbolsToFile(
      const std::string &fileName) const override {
    vt_->dump_profiler_symbols_to_file(inst_, fileName.c_str());
  }

private:
  const jsi_instrumentation_vtable *vt_;
  void *inst_;
};
...
facebook::jsi::Instrumentation &JsiAbiRuntime::instrumentation() {
  ...
  return facebook::jsi::Runtime::instrumentation();
}
