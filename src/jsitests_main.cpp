...
// Builds a config via the public C setters, hands it to create_runtime,
// and confirms a basic eval works. Does not (and cannot) verify that V8
// actually applied the requested flags — V8 platform-flag state is
// process-global and first-init-only, and these tests share a process.
TEST(JsiAbiConfig, RuntimeFromConfigEvaluates) {
  jsi_config cfg = v8_jsi_create_config();
  ASSERT_NE(cfg, nullptr);

  // Touch a representative subset of every field group to confirm linkage.
  v8_jsi_config_set_optimize_for_size(cfg, true);
  ...
  v8_jsi_config_set_sparkplug(cfg, false);
  v8_jsi_config_set_predictable(cfg, false);
  v8_jsi_config_set_always_compact(cfg, false);
...
// W9 query_interface negative-path test. Confirms that calling
// vt->query_interface with an IID that no implementation supports returns
// jsi_error_native and surfaces "Interface not supported" via the
// native-exception-message slot. Exercises the only path through which a
// non-JS error message flows back to the consumer.
TEST(JsiAbiQueryInterface, UnknownIidReturnsError) {
  const jsi_vtable *vt = get_jsi_abi_v8_vtable();
  ASSERT_NE(vt, nullptr);

  jsi_runtime *rt = vt->create_runtime(nullptr);
  ASSERT_NE(rt, nullptr);

  // Bogus IID — reserved-bit-pattern that will never collide with a real
  // well-known IID like JSI_IID_INSTRUMENTATION.
