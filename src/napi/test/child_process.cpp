...
ProcessResult spawnSync(std::string_view command, std::vector<std::string> args) {
  ...
  VerifyElseExit(CreatePipe(&out_read_handle.handle, &out_write_handle.handle, &handles_are_inheritable, 0));
  // Ensure the read handle to the pipe for STDOUT is not inherited.
  VerifyElseExit(SetHandleInformation(out_read_handle.handle, HANDLE_FLAG_INHERIT, 0));

  AutoHandle err_read_handle, err_write_handle;
  VerifyElseExit(CreatePipe(&err_read_handle.handle, &err_write_handle.handle, &handles_are_inheritable, 0));
  // Ensure the read handle to the pipe for STDERR is not inherited.
  VerifyElseExit(SetHandleInformation(err_read_handle.handle, HANDLE_FLAG_INHERIT, 0));
  ...
  VerifyElseExit(WAIT_OBJECT_0 == ::WaitForSingleObject(process_info.hProcess, INFINITE));
...
std::string readFromPipe(HANDLE pipeHandle) {
  ...
  for (;;) {
    DWORD bytesRead;
    BOOL isSuccess = ::ReadFile(pipeHandle, buffer, bufferSize, &bytesRead, nullptr);
...
void exitOnError(const char *message) {
  ...
  lpDisplayBuf = (LPVOID)LocalAlloc(LMEM_ZEROINIT, (lstrlenA((LPCSTR)lpMsgBuf) + lstrlenA(message) + 40));
  ::StringCchPrintfA(
      (LPSTR)lpDisplayBuf, LocalSize(lpDisplayBuf), "%s failed with error %d: %s", message, dw, lpMsgBuf);
