...
static napi_value Multiply(napi_env env, napi_callback_info info) {
  ...
  size_t byte_length;
  NODE_API_CALL(
      env, napi_get_arraybuffer_info(env, input_buffer, &data, &byte_length));
...
static napi_value External(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(
      env,
      napi_create_typedarray(
          env, napi_int8_array, nElem, output_buffer, 0, &output_array));
